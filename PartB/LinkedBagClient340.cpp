...
void displayBag(const std::unique_ptr<LinkedBag<std::string>>&);
...
int main() {
    ...
    std::unique_ptr<LinkedBag<std::string>> bag { std::make_unique<LinkedBag<std::string>>() };

    // A small std::vector of small objects to test the bag
...
void displayBag(const std::unique_ptr<LinkedBag<std::string>>& bag) {
    std::cout << "\n !Display bag: ";
